if (ghLikeFbase.sharedBy === 'www.giahuy.net') {
  ...
  function getLiked() {
    const elementId = '#gh-liked';
    const attribute = 'data-klik';
  ...
  function ghLike() {
    ...
    ref.once('value', snapshot => {
      ...
      if (localStorage.getItem(`ghLike${ghLikePostId}`) !== ghLikePostId) {
        ...
        document.querySelector('#gh-liked').setAttribute('data-teks', document.querySelector('#gh-liked').getAttribute('data-after'));
      }
      ref.set(likeCount);
      if (ghLikeFbase.abbreviation === '0') {
        document.querySelector('#gh-liked').setAttribute('data-klik', likeCount);
      } else if (ghLikeFbase.abbreviation === '1') {
        document.querySelector('#gh-liked').setAttribute('data-klik', intToString(likeCount));
      } else if (ghLikeFbase.abbreviation === '2') {
        document.querySelector('#gh-liked').setAttribute('data-klik', likeCount.toLocaleString('id-ID'));
      }
    });
  }
  (function() {
    const scriptUrl = 'https://cdn.firebase.com/js/client/2.2.1/firebase.js';
